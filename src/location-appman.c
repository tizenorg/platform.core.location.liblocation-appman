...
static char *__create_table = "create table if not exists LocationApp(package VARCHAR(64) PRIMARY KEY,\
			icon VARCHAR(256), app_id INTEGER, installed_date INTEGER, recently_used INTEGER, enable BOOL);";

static char *__default_item[] = {
	"insert into LocationApp (package, app_id, installed_date, recently_used, enable) values (\"org.tizen.setting\", 0, 0, 0, 1);",
	"insert into LocationApp (package, app_id, installed_date, recently_used, enable) values (\"kmcele1k0n.Internet\", 0, 0, 0, 1);",
};
...
int location_appman_open(void)
{
	...
	if (SQLITE_OK != rc) {
		...
		if (SQLITE_CANTOPEN == rc) {
			rc = sqlite3_open_v2(LOCATION_APPMAN_PATH_FULL, &db_handle, SQLITE_OPEN_FULLMUTEX | SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL);
			if (SQLITE_OK == rc) {
				rc = sqlite3_exec (db_handle, __create_table, NULL, NULL, NULL);
				if (SQLITE_OK != rc) {
					LOCATION_APPMAN_LOG("Failed to create table : Error[%s]\n", sqlite3_errmsg(db_handle));
					sqlite3_close (db_handle);
					db_handle = NULL;
					return LOCATION_APPMAN_ERROR_SQLITE_FAIL;
				}

				for (idx = 0; idx < sizeof(__default_item) / sizeof(char*); idx++) {
					rc = sqlite3_exec (db_handle, __default_item[idx], NULL, NULL, NULL);
					if (SQLITE_OK != rc) {
						LOCATION_APPMAN_LOG("Failed to insert default item : Error[%s]\n", sqlite3_errmsg(db_handle));
						sqlite3_close (db_handle);
						db_handle = NULL;
						return LOCATION_APPMAN_ERROR_SQLITE_FAIL;
					}
